...
## Flag-based ranks + locked contracts (round 60)
- SHAREHOLDER can't be a rankOf(jobsDone) threshold (that would auto-grant by contract count). It's a flag (prof.shareholder) with RANKS[7]=['SHAREHOLDER', Infinity] so rankOf never returns it; a rankIx(prof) helper is used ONLY at display sites (publicPlayer/report/stats). The one gameplay gate (the market) checks prof.shareholder directly — so no need to thread rankIx through every unlock call site.
- A locked/mandatory job's flags must be carried into prof.job when TAKEN — jobTake originally built a fresh {kind,mat,need,pay,tier,n,slot} object and dropped `locked`/`final`, which would have let proximity referrals satisfy the mandatory refer. Always copy the discriminating flags when materialising an active job from an offer.
- Growth gate design: mandatory refer contracts require an invite-link recruit (jobEvent viaLink flag set only on the ?by= credit path); proximity credit still satisfies OPTIONAL mid-game refer contracts. This makes the gate actually drive sharing rather than being auto-satisfied at busy sites.
