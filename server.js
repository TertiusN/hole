...
const RELEASES = [
  ...
  ['v43', 'RE: HEADCOUNT', ['A fault by which stationary employees appeared to cease existing has been resolved. You were always present. We always knew where.']],
  ['v42', 'RE: POWER & CLEARANCE', ['Ascent is now billed by the second. The company reclaims what it lends.', 'A hands-free light source is available to those who have gone deep enough to require one.', 'Demolition stock is now visible to all and issued to few.']],
  ['v41', 'RE: ARRIVAL & EGRESS', ['Delivery coordinates corrected: you will no longer be released into open air above a grave.', 'A means of vertical escape is stocked, for staff enclosed by the enthusiasm of their colleagues.', 'Demolition now requires a licence. Leased units report to their supervisor on arrival.']],
  ['v40', 'RE: AUTOMATION', ['Surplus autonomous units have been retired; the surface was becoming a congregation. Leased units continue regardless of your attendance.']],
  ['v39', 'RE: THE DEEP LEDGER', ['A rare mineral has been catalogued below sixty meters. Its worth is considerable. Should you hear it ring a second time, do not file a report.', 'Commerce may now be established at the foot of a shaft. The company follows its money down.']],
  ['v38', 'RE: MORALE', ['The roster on the company report has been replaced with a map.', 'Provisions are now stocked for consumption on the clock. They are nutritionally inert. Consume them regardless.']],
  ['v37', 'RE: SITE SECURITY', ['Measures have been taken against those who would interfere with the process.', 'The count only ever descends. This is not a feature. It is load-bearing.']],
  ['v36', 'RE: APPROVED EXPRESSIONS', ['Personnel may now display a reviewed range of sentiment above the hard hat. Interns may wave; the remainder is earned.', 'Field telemetry has been appended to your file. You are not cleared for all of it.']],
  ['v35', 'RE: SIGNAGE', ['Painted notices are available to promoted staff. Twelve characters. The company reads each one before you do. Interns are not issued paint.']],
  ['v34', 'RE: THE BOARD', ['Contract work is now posted by your marker. Complete it to ascend. It is generated by a department nobody has yet been able to locate.']],
  ['v33', 'RE: SIGNALS', ['A shell is now stocked that writes a star the whole neighbourhood can see.', 'A compass has been fitted beside the clock. It will not help you leave.']],
  ['v32', 'RE: FILES', ['Any employee’s file may now be pulled — employed or interred.', 'The company’s full name lies buried below four meters. It tested poorly. Leave it.']],
  ['v31', 'RE: ADDRESS', ['The planet has been given an address. This does not make it somewhere to live.']],
  ['v30', 'RE: THE PAPER TRAIL', ['The company has begun interring its correspondence in the deep. The further down you go, the more it concedes.', 'Wealth recovered from graves now reaches your wallet. We are assured this was always the arrangement.']],
  ['v1.0', 'FIRST SHIFT', ['Nine hundred and ninety-nine billion units of earth. One shared hole. Your instrument is a stick; the board reviewed the stick and affirmed the stick.', 'There are biomes above and a great deal of nothing below. At the very bottom, sealed into the bedrock, there is a door. It does not open. Do not dig toward it.']],
];
function renderReleases() {
  const entries = RELEASES.map(([ver, title, items]) => `
<div class="panel" style="margin-bottom:14px"><h2>${esc(title)} <span class="dim" style="float:right">${esc(ver)}</span></h2>
